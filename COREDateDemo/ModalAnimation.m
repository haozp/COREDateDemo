...
-(void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    if (self.type == AnimationTypePresent) {
        ...
        NSDictionary *views = NSDictionaryOfVariableBindings( modalView);
...
//         _constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-30-[modalView]-30-|" options:0 metrics:nil views:views];
        [containerView addConstraints:_constraints];
        
        //Move off of the screen so we can slide it up
        CGRect endFrame = modalView.frame;
        modalView.frame = CGRectMake(endFrame.origin.x, containerView.frame.size.height-300, endFrame.size.width, endFrame.size.height);
        ...
        [UIView animateWithDuration:[self transitionDuration:transitionContext] delay:0.0 usingSpringWithDamping:0.8 initialSpringVelocity:1.0 options:0 animations:^{
            ...
            fromViewController.view.transform = CGAffineTransformMakeTranslation(-120, 0);
